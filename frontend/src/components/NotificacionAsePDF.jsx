...
export default function NotificacionAsePDF({ data }) {
    ...
    <Document>
      <Page size="A4" style={S.page}>
        ...
        <View style={S.titleBar}>
          <View>
            <Text style={S.titleMain}>NOTIFICACIÓN AL ASEGURADO</Text>
        ...
        <View style={S.content}>
          ...
          <View style={S.destinatario}>
            <Text style={S.destNombre}>Señor(a) {nombreAseg}</Text>
            {asegurado.tipo_identificacion_nombre && (
              <Text style={S.destDato}>{asegurado.tipo_identificacion_nombre}: {asegurado.cedula || '—'}</Text>
            )}
            {!asegurado.tipo_identificacion_nombre && asegurado.cedula && (
              <Text style={S.destDato}>C.I.: {asegurado.cedula}</Text>
            )}
            {asegurado.cua && <Text style={S.destDato}>CUA: {asegurado.cua}</Text>}
            {asegurado.direccion && <Text style={S.destDato}>Dirección: {asegurado.direccion}</Text>}
            <Text style={S.presente}>Presente.-</Text>
          </View>

          {/* Referencia */}
          <View style={S.refLine}>
            <Text><Text style={S.refLabel}>Ref.:</Text><Text style={S.refValue}> RESPUESTA A SU SOLICITUD DE RECTIFICACIÓN</Text></Text>
          </View>
          ...
          {/* Tabla FPC */}
          <View style={S.tHead}>
            <Text style={[S.tHCell, { width: '30%' }]}>N° FPC</Text>
            <Text style={[S.tHCell, { width: '25%' }]}>Período</Text>
            <Text style={[S.tHCell, { width: '45%' }]}>Observación</Text>
          </View>
          {fpcs.length === 0 ? (
            <View style={S.tRow}>
            ...
            <View key={idx} style={[S.tRow, idx % 2 === 1 ? S.tRowAlt : {}]}>
              <Text style={[S.tCell, { width: '30%', fontFamily: 'Helvetica-Bold', fontSize: 7 }]}>{f.numero || '—'}</Text>
              <Text style={[S.tCell, { width: '25%' }]}>{f.periodo || '—'}</Text>
